...
class CreadorMemes {
  ...
  crearCanvas() {
    let alto = Math.min(480, ANCHO_DISPOSITIVO - 30),
      ancho = Math.min(640, ANCHO_DISPOSITIVO - 30);
    ...
    if (this.imagen.files && this.imagen.files[0]) {
      ...
      lector.onload = () => {
        ...
        image.onload = () => {
          ...
          let tamagnioFuente =
            (((this.imagenCanvas.width + this.imagenCanvas.height) / 2) * 4) /
            100;
          ...
          let textoArriba = this.textoSuperior.value.toUpperCase(),
            textoAbajo = this.textoInferior.value.toUpperCase();

          contexto.strokeText(
            textoArriba,
            this.imagenCanvas.width / 2,
            this.imagenCanvas.height * (5 / 100)
          );
          contexto.fillText(
            textoArriba,
            this.imagenCanvas.width / 2,
          ...
          contexto.strokeText(
            textoAbajo,
            this.imagenCanvas.width / 2,
            this.imagenCanvas.height * (90 / 100)
          );
          contexto.fillText(
            textoAbajo,
            this.imagenCanvas.width / 2,
          ...
          contexto.fillText(
            waterMark,
            this.imagenCanvas.width -
              contexto.measureText(waterMark).width -
              10,
            this.imagenCanvas.height - 20
          );
        };
  ...
  redimensionarCanvas(alto, ancho) {
    ...
    while (
      alto > Math.min(1000, ANCHO_DISPOSITIVO - 30) &&
  ...
  descargarMeme() {
    ...
    atributo.value = fuenteImagen.replace(
      /^data:image\/[^;]/,
